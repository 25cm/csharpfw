...
namespace FukjTabletSystem.Application.ApplicationLogic.Kensa.GaikanKensaTabPage
{
    #region 入力インターフェイス定義
    ////////////////////////////////////////////////////////////////////////////
    ...
    /// </history>
    ////////////////////////////////////////////////////////////////////////////
    interface IKakuteiBtnClickALInput : IBseALInput, IUpdateShokenKekkaTblBLInput, IUpdateShokenKekkaHosokuTblBLInput
    {
    }
    #endregion
    ...
    class KakuteiBtnClickApplicationLogic : BaseApplicationLogic<IKakuteiBtnClickALInput, IKakuteiBtnClickALOutput>
    {
        ...
        /// 機能名
        /// </summary>
        private const string FunctionName = "GaikanKensaTabPage：KakuteiBtnClick";
            ...
            try
            {
                try
                {
                    ...
                    if (input.ShokenKekkaTbl.Count > 0)
                    {
                        ...
                        if (input.ShokenKekkaTbl.Count > 0)
                        {
                            IUpdateShokenKekkaTblBLOutput blShokenOutput = new UpdateShokenKekkaTblBusinessLogic().Execute(input);
                        }

                        if (input.ShokenKekkaHosokuTbl.Count > 0)
                        {
                            IUpdateShokenKekkaHosokuTblBLOutput blHosokuOutput = new UpdateShokenKekkaHosokuTblBusinessLogic().Execute(input);
                        }
                        ...
                        if (input.ShokenKekkaTbl.Count > 0)
                        {
                            ...
                            // 外観検査結果登録
                            IUpdateGaikanKensaKekkaByKensaIraiKeyBLInput gaikankensaInput = new UpdateGaikanKensaKekkaByKensaIraiKeyBLInput();
                            gaikankensaInput.IraiHoteiKbn = input.ShokenKekkaTbl[0].KensaIraiShokanIraiHoteiKbn;
                            gaikankensaInput.IraiHokenjoCd = input.ShokenKekkaTbl[0].KensaIraiShokenIraiHokenjoCd;
                            gaikankensaInput.IraiNendo = input.ShokenKekkaTbl[0].KensaIraiShokenIraiNendo;
                            gaikankensaInput.IraiRenban = input.ShokenKekkaTbl[0].KensaIraiShokenIraiRenban;

                            new UpdateGaikanKensaKekkaByKensaIraiKeyBusinessLogic().Execute(gaikankensaInput);
                        }
                    }
